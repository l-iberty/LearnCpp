...
// 针对原生指针而设计的"偏特化版" (particial specialization)
template <class T>
struct iterator_traits<T*> {
  typedef random_access_iterator_tag iterator_category; // 注意: 原生指针是一种Random Access Iterator
  typedef T                          value_type;
...
// 针对原生const指针设计的"偏特化版" (particial specialization)
template <class T>
...
// 为什么只给distance()设计针对Input Iterator和Random Access Iterator的版本? 因为只有
// Random Access Iterator可以直接使用"-"计算距离, 其他Iterator的计算距离的方式完全一致,
// 所以只需设计一份针对Input Iterator的版本, 通过各种Iterator之间的继承关系实现调用传递.
template <class InputIterator>
inline iterator_traits<InputIterator>::difference_type
__distance(InputIterator first, InputIterator last, input_iterator_tag) {
  iterator_traits<InputIterator>::difference_type n = 0;
  while (first != last) {
    ++first; ++n; // 除了RandomAccessIterator外的iterator在计算距离时都只能逐一累计, T(n)=O(n)
  }
  return n;
}
...
__distance(RandomAccessIterator first, RandomAccessIterator last,
  ...
  return last - first; // 因为RandomAccessIterator重载了减号运算符"-", 所以可以这么写, T(n)=O(1)
}
...
// 注意: 这是针对Input Iterator而设计的版本, 但是针对ForwardIterator的advance()没有给出,
// 也就是第三个参数类型是forward_iterator_tag的版本. 事实上, 针对ForwardIterator的代码和
// 针对Input Iterator的完全一致. 根据继承关系, 如果第三个参数是forward_iterator_tag, 并且
// 没有给出专门的advance(), 那么在重载解析时就可以将调用传递到这个针对Input Iterator的版本,
// 巧妙地消除了重复代码.
template <class InputIterator, class Distance>
...
// 按说advance()既然可以接受各种类型的迭代器, 就不应该将其型别参数命名为InputIterator.
// 这其实是STL的一个命名规则: 以算法所能接受之最低阶迭代器类型, 来为其迭代器型别参数命名.
template <class InputIterator, class Distance>
