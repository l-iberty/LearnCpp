...
void * __malloc_alloc_template<inst>::oom_malloc(size_t n)
{
    ...
    for (;;) {  // 不断尝试释放、分配、再释放、再分配...
        ...
        if (0 == my_malloc_handler) { __THROW_BAD_ALLOC; } // 如果客户端未指定“内存不足处理例程”, 一旦内存分配失败就会抛出异常
        (*my_malloc_handler)(); // 调用处理例程, 可能做一些内存释放工作
        result = malloc(n);
...
// 以上第一级配置器__malloc_alloc_template,
// 第一级配置器以malloc(),free(),realloc()执行内存的分配、释放、重分配操作,
// 并实现出类似C++ new_handler的机制.
...
# endif
  // 第二级配置器维护16个free_lists, 各自管理的大小分别为8,16,24,32,...,128 bytes的区块.
  // FREELIST_INDEX()根据区块的大小, 决定使用第i个free_list. 例如: bytes=8, 则i=(((8)+8-1)/8-1)=0, 使用第0号free_list;
  // bytes=30, i=(((30)+8-1)/8-1)=3, 使用第3号free_list.
...
public:
  ...
  static void * allocate(size_t n)
  {
    obj * __VOLATILE * my_free_list; // obj * volatile * my_free_list;
    obj * __RESTRICT result; // obj * result;

    // 大于128就调用第一级配置器
    if (n > (size_t) __MAX_BYTES) {
        return(malloc_alloc::allocate(n));
    }
    // 根据字节数n, 选择一个适当的free_list
...
#       endif
  ...
  static void deallocate(void *p, size_t n)
  {
    ...
    // 大于128说明内存是第一级配置器分配的, 所以调用第一级配置器
    if (n > (size_t) __MAX_BYTES) {
...
__default_alloc_template<threads, inst>::chunk_alloc(size_t size, int& nobjs)
{
    ...
    size_t bytes_left = end_free - start_free; // 内存池剩余空间. 首次调用时end_free和start_free都是NULL, bytes_left=0
    ...
    if (bytes_left >= total_bytes) {
        ...
        // 让内存池的残余零头还有利用价值
        if (bytes_left > 0) {
            // bytes_left肯定是8的倍数, 所以一定可以找到合适的free_list, 将内存池的残余空间编入此free_list,
            // 于是这个free_list就被扩充了.
            obj * __VOLATILE * my_free_list =
        ...
        if (0 == start_free) { // malloc failed
            int i;
            obj * __VOLATILE * my_free_list, *p;
            // Try to make do with what we have.  That can't
            // hurt.  We do not try smaller requests, since that tends
            // to result in disaster on multi-process machines.
            // 走到这里说明malloc已经失败, 即process heap中已经没有足够的空间了, chunk_alloc()只
            // 能依次寻找free_list中的空闲区快. 如果能找到空闲区块, 就把这块空闲区域用start_free和
            // end_free标记起来, 用作新的内存池, 然后基于这个新的内存池递归地调用chunk_alloc()
            ...
            end_free = 0;	// In case of exception. 到处都没有内存可用了
            // 调用第一级配置器, 看看oom_函数能否帮点忙. 如果可能的话, malloc_handler会释放一些内存
            // 以供此处使用; 否则发出bad_alloc异常.
...
{
    int nobjs = 20;
    // 调用chunk_alloc(), 尝试取得nobjs个大小为n的区块作为free_list的新节点
    // 注意nobjs是pass by reference, 所以chunk_alloc()会内部修改之, 以告知
    // 调用者实际分配的区块数量.
    ...
    // 如果只获得了一个区块, 就将其直接返回给客户端使用, free_list无新节点
    ...
    /* Build free list in chunk */
      result = (obj *)chunk;
      // chunk的前n字节的区域返回给客户端使用, 所以建立free_list时需要跳过n字节
      *my_free_list = next_obj = (obj *)(chunk + n);
      for (i = 1; ; i++) { // 从1开始, 因为第0个返回给客户端使用
