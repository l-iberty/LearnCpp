...
struct __deque_iterator {
  ...
  self& operator--() {
    if (cur == first) {    // 如果以达到当前缓冲区的头端,
      set_node(node - 1);  // 就切换至前一个节点(亦即缓冲区)
      cur = last;          //  的尾端
    }
    --cur;                 // 切换至前一个元素 (这里的"--"是针对原生指针)
    return *this;
  }
  self operator--(int) {
    self tmp = *this;
  ...
  self& operator+=(difference_type n) {
    difference_type offset = n + (cur - first); // 目标位置相对于当前缓冲区起始位置的偏移量
  ...
  reference operator[](difference_type n) const {
    return *(*this + n); // 调用operator+, 然后转入operator+=
  }
...
protected:                      // Internal typedefs
  typedef pointer* map_pointer; // map_pointer 即 T**
...
public:                         // Basic accessors
  ...
  size_type size() const { return finish - start;; } // 调用 __deque_iterator<>::operator-
...
public:                         // push_* and pop_*
  ...
  void push_back(const value_type& t) {
    if (finish.cur != finish.last - 1) {
      // finish是指向最后一个缓冲区的迭代器
      // 如果finish.cur未指向缓冲区中的最后一个位置, 就在finish.cur所指的位置
      // 构造一个值为t的元素
      construct(finish.cur, t);
      // finish.cur指向缓冲区的下一个可以放元素的有效位置
      ++finish.cur; // finish.cur是T*类型, "++"是原生指针的递增
    }
    else {
      // 最后一个缓冲区已经满载
      push_back_aux(t);
  ...
  void push_front(const value_type& t) {
    if (start.cur != start.first) { // start.cur之前尚有空闲位置
      construct(start.cur - 1, t);
      --start.cur;
    }
    else {
      // 第一次调用push_front()时, start.cur==start.first,
      // 即start.cur之前没有空闲位置
      push_front_aux(t);
    }
  }
  ...
  void pop_back() {
    if (finish.cur != finish.first) {
      --finish.cur; // finish.cur指向的是缓冲区中下一个空闲位置, 而不是最后一个元素所在的位置, 所以要先--
      destroy(finish.cur);
    }
    else {
      pop_back_aux();
  ...
  void pop_front() {
    if (start.cur != start.last - 1) {
      // 如果start.cur未指向缓冲区的最后一个位置, 则直接销毁start.cur所指
      // 位置上的元素, 再将start.cur后移
      destroy(start.cur);
      ++start.cur;
    }
    else {
...
public:                         // Insert
  ...
  iterator insert(iterator position, const value_type& x) {
    if (position.cur == start.cur) { // 如果插入点是deque最前端
      ...
      return start; // push_front()之后, *start得到的是值为x的元素
    }
    else if (position.cur == finish.cur) { // 如果插入点是deque最后端
      push_back(x);                        // 交给push_back()去做
      iterator tmp = finish; // push_back()之后, finish.cur指向缓冲区中的下一个空闲位置
      --tmp;                 // finish.cur-1 才是元素x, 返回的迭代器要指向x, 所以需要tmp
      return tmp;
    }
    else {
...
public:                         // Erase
  iterator erase(iterator pos) {
    ...
    // 在连续空间中删除元素时需要进行元素移动, if-else逻辑和insert()一样,
    // 都是为了尽可能减少元素移动次数
    if (index < (size() >> 1)) {
...
protected:                      // Allocation of map and nodes
  ...
  void reserve_map_at_back (size_type nodes_to_add = 1) {
    if (nodes_to_add + 1 > map_size - (finish.node - map))
      // 如果map尾部的备用节点太少, 就重换一个map
      reallocate_map(nodes_to_add, false); // add_at_front=false
  }

  void reserve_map_at_front (size_type nodes_to_add = 1) {
    if (nodes_to_add > start.node - map)
      // 如果map头部的备用节点太少, 就重换一个map
      reallocate_map(nodes_to_add, true); // add_at_front=true
  }
...
void deque<T, Alloc, BufSize>::clear() {
  for (map_pointer node = start.node + 1; node < finish.node; ++node) { // 如果map中只有1或2个节点就不会进入循环
    destroy(*node, *node + buffer_size());            // 元素析构
    data_allocator::deallocate(*node, buffer_size()); // 释放缓冲区内存
  }

  if (start.node != finish.node) { // map中至少有2个节点(缓冲区), start.node指向第一个, finish.node指向最后一个
    destroy(start.cur, start.last);    // 析构第一个缓冲区中的所有元素, 但不释放内存
    // 析构最后一个缓冲区中的所有元素(注意最后缓冲区可能不是满载, 所以没有放到上面的循环中处理), 并释放内存
  ...
  // clear()之后所有元素均被析构, 除了第一个缓冲区外, 剩余缓冲区的内存被释放.
  // 即, deque中只剩一个空缓冲区
}
...
void deque<T, Alloc, BufSize>::push_back_aux(const value_type& t) {
  value_type t_copy = t;
  reserve_map_at_back();
  *(finish.node + 1) = allocate_node(); // 往后分配一个新节点 (缓冲区)
  __STL_TRY {
    construct(finish.cur, t_copy); // 此时finish.cur指向最后一个缓冲区中下一个可以放置元素的位置
    // 设置finish指向新分配的节点
...
void deque<T, Alloc, BufSize>::push_front_aux(const value_type& t) {
  ...
  *(start.node - 1) = allocate_node(); // 往前分配一个新节点
  __STL_TRY {
    // 设置start指向新分配的节点, start.cur指向新缓冲区的最后一个位置
    start.set_node(start.node - 1);
    start.cur = start.last - 1;
    // 在新分配的缓冲区的最后一个位置上构造值为t的元素
...
void deque<T, Alloc, BufSize>::pop_back_aux() {
  deallocate_node(finish.first); // finish.node指向的缓冲区是空的, finish.cur指向的位置是一个空闲位置
  finish.set_node(finish.node - 1);
  finish.cur = finish.last - 1; // 现在的finish.cur才是指向最后deque中的最后一个元素,
  destroy(finish.cur);          // 把它销毁后, 这个位置就变成了一个空闲位置, 下次push_back()时新元素就放在这里
}
...
void deque<T, Alloc, BufSize>::pop_front_aux() {
  destroy(start.cur); // 销毁start.cur位置上的元素
  deallocate_node(start.first); // 释放start所指的整个缓冲区
  // start指向后一个节点, start.cur指向缓冲区第一个位置
  start.set_node(start.node + 1);
  start.cur = start.first;
}      
...
deque<T, Alloc, BufSize>::insert_aux(iterator pos, const value_type& x) {
  difference_type index = pos - start;
  value_type x_copy = x;
  if (index < size() / 2) {      // 如果插入点pos之前的元素个数比较少
    push_front(front());         // 在最前端将第一个元素复制一份
    iterator front1 = start;     // 以下进行元素移动, 将pos指向的位置空出来, 以便放入元素x
    ++front1;
  ...
  else {                         // 如果插入点pos之前的元素个数比较多, 后面的元素个数比较少
    push_back(back());           // 在最尾端将最后一个元素复制一份
  ...
  *pos = x_copy;
  return pos;
  // 在连续空间中插入元素必然发生元素移动, if-else分支的目的是尽可能减少元素的移动次数
}
...
void deque<T, Alloc, BufSize>::reallocate_map(size_type nodes_to_add,
  ...
  if (map_size > 2 * new_num_nodes) {
    ...
    if (new_nstart < start.node) {
      ...
      // copy()可以有两种实现方式, 一是直接memmove()——如果拷贝对象没有重载"=";
      // 二是通过"="逐个正向拷贝——如果拷贝对象重载了"=".
      // 对于上图所示的3个指针的相对位置, 这两种实现方式都不会因缓冲区重叠
      // 而出错;
      // 但若new_nstart位于[start.node,finish.node], 如else分支所示,
      // 那么copy()的第二种实现方式(非memmove)就会因缓冲区重叠而出错, 所以要用copy_backward()进行逆向拷贝
    }
    else {
      // 4个指针之间的相对位置:
      //            new_nstart     new_nstart+old_num_nodes
      //                |                 |
      //        |                |
      //    start.node      finish.node
      //
      // new_nstart位于[start.node,finish.node]
